...
export default function RoomsView({ 
  user, // Added user object to access user.id for fetching connections
  userRooms, 
  setUserRooms,
  subjectRoomsData, 
  subjectRoomsLoading, 
  ...
  // --- NEW STATE FOR INVITING CONNECTIONS ---
  ...
  // --- EFFECT TO FETCH FOLLOWING LIST WHEN MODAL OPENS ---
  useEffect(() => {
    ...
    fetchFollowing();
  }, [showCreateRoom, user?.id]);

  // --- UPDATED ROOM CREATION METHOD ---
  ...
  name: roomName, passKey: useGeneratedKey ? null : roomPassKey, invitedUsers
      ...
      // Updates the UI array instantly without reloading everything!
      if (setUserRooms) {
        setUserRooms(prev => [...prev, createdRoom]);
      }
      ...
      // Give the user a moment to see the passkey before hiding the modal
      setTimeout(() => {
    ...
    <div className="rooms-view">
      ...
      {activeSubTab === "browse" && (
        <div className="browse-rooms-content">
          <div className="browse-search-bar" style={{ display: "flex", gap: "12px", marginBottom: "24px", flexWrap: "wrap" }}>
            <div style={{ flex: 2, minWidth: "200px" }}>
              <input
                ...
                style={{ width: "100%", padding: "10px 14px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.2)", background: "rgba(255,255,255,0.1)", color: "var(--text-main)", fontSize: "14px" }}
              />
            </div>
            ...
            <div style={{ flex: 1, minWidth: "150px" }}>
              <select
                ...
                style={{ width: "100%", padding: "10px 14px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.2)", background: "rgba(255,255,255,0.1)", color: "var(--text-main)", fontSize: "14px", cursor: "pointer" }}
              >
            ...
            {(roomSearchQuery || selectedMajorFilter !== "all") && (
              <button
                onClick={() => { setRoomSearchQuery(""); setSelectedMajorFilter("all"); }}
                style={{ padding: "10px 16px", borderRadius: "8px", background: "rgba(100,118,175,0.3)", border: "1px solid #6476af", color: "var(--text-main)", cursor: "pointer", fontSize: "13px" }}
              >
      ...
      {showCreateRoom && (
        <div className="modal-overlay" onClick={() => setShowCreateRoom(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <label style={{display: "block", marginBottom: "5px"}}>{t?.('profile.createRoomModal.roomNameLabel') || "Room Name"}</label>
            <input
              type="text"
              value={roomName}
            ...
            {/* Render connections list if following anyone */}
            {following.length > 0 && (
              <div style={{marginBottom:"15px"}}>
                <label style={{fontWeight: "bold"}}>{t?.('profile.createRoomModal.inviteLabel') || "Invite your connections:"}</label>
                <div style={{maxHeight:"150px", overflowY:"auto", marginTop:"8px", display:"flex", flexDirection:"column", gap:"8px", padding: "5px", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "6px"}}>
                  {following.map(u => (
            ...
            {roomFeedback && (
              <p style={{color: roomFeedback.includes("created") || roomFeedback.includes("is:") ? "lightgreen" : "#fc0c0ce9", marginBottom:"10px"}}>
                {roomFeedback}
              </p>
            )}

            <div style={{display:"flex", justifyContent:"flex-end", gap:"10px", marginTop:"15px"}}>
              <button onClick={() => setShowCreateRoom(false)} style={{padding:"6px", width:"70px", backgroundColor:"#fc0c0ce9", color:"white", borderRadius:"6px", border:"none", cursor: "pointer"}}>
                {t?.('profile.createRoomModal.cancel') || "Cancel"}
              </button>
              <button onClick={handleCreatePrivateRoom} disabled={!roomName.trim() || roomLoading} style={{padding:"6px", width:"70px", backgroundColor:"#6476af", color:"white", borderRadius:"6px", border:"none", cursor: "pointer"}}>
                {roomLoading ? (t?.('profile.createRoomModal.creating') || "Creating...") : (t?.('profile.createRoomModal.create') || "Create")}
              </button>
            </div>
