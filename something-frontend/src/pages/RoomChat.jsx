    ...
    <div className="roomchat-header">
      ...
      {isAdmin && (
        <div className="roomchat-admin-actions">
          <button onClick={() => setShowMembers(true)} className="roomchat-admin-button">
          ...
          <button onClick={() => {
            if (window.confirm(t("roomChat.deleteRoomConfirm"))) {
              api.delete(`/rooms/private/${roomId}`).then(() => navigate("/profile"));
            }
          ...
          {room?.passKey && (
            <div style={{
              ...
              <code style={{
                background: "rgba(255,255,255,0)", padding: "2px 8px", borderRadius: "4px",
                letterSpacing: "2px", color: "var(--text-main)", fontSize: "14px",
                fontWeight: "bold", userSelect: "all",
                fontFamily: '"OCR-A", "Courier New", "JetBrains Mono", monospace'
              }}>
                {room.passKey}
              </code>
              <button onClick={() => { navigator.clipboard.writeText(room.passKey); alert("Passkey copied!"); }}
                className="roomchat-admin-button">
                <Copy size={16} />
              </button>
            </div>
              ...
              <div key={msg.id} className={`roomchat-message-wrapper ${msg.senderId === currentUser?.id ? "roomchat-message-right" : "roomchat-message-left"}`}>
                ...
                <div className={`roomchat-message-bubble ${msg.senderId === currentUser?.id ? "roomchat-message-bubble-right" : "roomchat-message-bubble-left"}`}>
                  ...
                  {editingMessage?.id === msg.id ? (
                    <div>
                      <input value={editContent} onChange={(e) => setEditContent(e.target.value)} className="roomchat-edit-input" />
        ...
        <div className="roomchat-modal" onClick={(e) => e.stopPropagation()}>
          <div className="roomchat-modal-header">
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              <button
                onClick={() => { if (!showInvite) openInvite(); else setShowInvite(false); }}
                className="roomchat-admin-button"
                style={{ fontSize: "12px" }}
              >
              ...
              {followList.length === 0
                ...
                : followList.map(user => (
                  <div key={user.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0" }}>
                    ...
                    <button onClick={() => handleInvite(user.id)} className="roomchat-make-admin-button">Add</button>
                  </div>
                ))
              }
            </div>
